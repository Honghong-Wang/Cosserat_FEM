...
def mul1(rmat, k, db):
    c = np.zeros((6, 6))
    f = rmat @ skew(k) @ db @ rmat
    c[0: 3, 0: 3] = f
    return c


def mul3(db, rmat, rmatds):
    c = np.zeros((6, 6))
    f = rmat @ db @ rmatds
    c[3: 6, 3: 6] = f
    return c
...
def mul5(rmat, k, rmatds, db):
    c = np.zeros((6, 6))
    f = rmat @ skew(k) @ rmatds
    c[0: 3, 0: 3] = f
    return c
